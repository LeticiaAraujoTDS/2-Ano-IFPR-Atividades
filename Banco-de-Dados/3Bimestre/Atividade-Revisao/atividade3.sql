...
select funcionarios.primeiroNome, departamentos.nome as nomeDepartamento, funcionarios.funcao
from funcionarios join departamentos on funcionarios.codigo_departamento = departamentos.coddept;

-- H) Liste todos departamentos com seus respectivos gerentes
select departamentos.nome as departamento, funcionarios.primeironome, funcionarios.ultimonome
from departamentos join funcionarios on departamentos.codigo_funcionario_gerente = funcionarios.codfunc;

-- I) Liste o valor da folha de pagamento de cada departamento (nome)
select departamentos.nome as departamento, sum(funcionarios.salario) as folha_pagamento
from funcionarios join departamentos on funcionarios.codigo_departamento = departamentos.coddept
group by departamentos.nome;

-- J) Liste os departamentos dos funcionários que têm a função de supervisor
select departamentos.nome as departamento
from funcionarios join departamentos on funcionarios.codigo_departamento = departamentos.coddept
...
select departamentos.nome as departamento, funcionarios.primeironome, funcionarios.ultimonome
from departamentos join funcionarios on funcionarios.codigo_departamento = departamentos.coddept
...
from funcionarios where segundoNome is null;

-- P) Liste o menor salário pago pela empresa em cada departamento
select departamentos.nome as departamento, min(funcionarios.salario) as menor_salario
from funcionarios join departamentos on funcionarios.codigo_departamento = departamentos.coddept
group by departamentos.nome;